...
export default async function crawl () {
  ...
  // grab badges
  await page.goto('https://stackoverflow.com/users/904956/wesley-lemahieu?tab=badges&sort=class', options);
  const selector2 = 'div#user-tab-badges div.grid--item a';
  await page.waitForSelector(selector2, { visible: true, timeout: 20000 });
  // console.log('Selector found!');
  const badges = await page.$$eval(selector2, items => items.map(item => ({
